...
async function main() {
  ...
  await loadLocalEnv(LOCAL_ENV_PATH);
  const config = await readJson(CONFIG_PATH);
  const apiKey = process.env.OPENAI_API_KEY;
  const model = options.model || config.model || "gpt-4o-mini-tts";
  const voice = options.voice || config.voice || "coral";
  ...
  if (!apiKey && !options.dryRun) {
    throw new Error("OPENAI_API_KEY is required. Re-run with OPENAI_API_KEY set, or use --dry-run to preview.");
  }
  ...
  for (const clip of config.clips) {
    ...
    if (options.dryRun) {
      console.log(`[dry-run] ${clip.id} -> ${path.relative(ROOT, outputPath)}`);
      continue;
    }
    ...
    console.log(`generate ${clip.id}`);
    const audio = await createSpeech({
      apiKey,
      model,
      voice,
      format,
      instructions: clip.instructions || config.instructions,
      text: clip.text,
    });
  ...
  const manifest = {
    ...
    disclosure: "Voice prompts are AI-generated.",
    model,
    voice,
    format,
    clips,
